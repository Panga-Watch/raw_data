...
```{r}
vms_text_df<- survey %>% 
  select(survey_id, c(50:53,197:200)) %>% 
  mutate(positive_vms1 = ifelse(is.na(positive_tracking_tech_one), positive_one, positive_tracking_tech_one),
         positive_vms2 = ifelse(is.na(positive_tracking_tech_two), positive_two, positive_tracking_tech_two),
         negative_vms1 = ifelse(is.na(negative_tracking_tech_one), negative_one, negative_tracking_tech_one),
         negative_vms2 = negative_two) %>% 
  select(survey_id, positive_vms1, positive_vms2, negative_vms1, negative_vms2)
...
```{r}
pct_catch<-survey %>% 
  select(survey_id, c(78:80, 221:223)) 
```
...
problems<-survey %>% 
  ...
  select(survey_id, starts_with("fishery_problem"), starts_with("rank_"), catch_trends)
```
